...
from utils_weather_5 import ngsimDataset, maskedNLL, maskedMSE, maskedNLLTest
...
trSet = ngsimDataset("TrainSet_weather_5.npy")
valSet = ngsimDataset("ValSet_weather_5.npy")
    ...
    for i, data in enumerate(trDataloader):
        ...
        if args["use_maneuvers"]:
            fut_pred, lat_pred, lon_pred = net(
                hist, weather, nbrs, mask, lat_enc, lon_enc
            )
            ...
            if epoch_num < pretrainEpochs:
                ...
                avg_lat_acc += (
                    torch.sum(
                        torch.max(lat_pred.data, 1)[1] == torch.max(lat_enc.data, 1)[1]
                    )
                ).item() / lat_enc.size()[0]
                avg_lon_acc += (
                    torch.sum(
                        torch.max(lon_pred.data, 1)[1] == torch.max(lon_enc.data, 1)[1]
                    )
                ).item() / lon_enc.size()[0]
        else:
...
torch.save(net.state_dict(), "trained_models/cslstm_m_weather_5.tar")
